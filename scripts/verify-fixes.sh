...
case "$ROOT_IMPL" in
  sukisu-ultra)
    ...
    grep -Fq 'KSU_LEGACY_PRCTL_OPTION' "$KSU_DIR/hook/lsm_hook.c" || die "legacy manager prctl bootstrap is missing"
    grep -Fq 'LSM_HOOK_INIT(task_prctl, ksu_task_prctl)' "$KSU_DIR/hook/lsm_hook.c" || die "legacy manager prctl hook is not registered"

    test -s "$KERNEL_ROOT/fs/exec.c" || die "fs/exec.c missing"
    grep -Fq 'ksu_handle_execveat(&fd, &filename, &argv, &envp, &flags);' "$KERNEL_ROOT/fs/exec.c" || die "manual execve hook missing"
    grep -Fq 'ksu_handle_faccessat(&dfd, &filename, &mode, NULL);' "$KERNEL_ROOT/fs/open.c" || die "manual faccessat hook missing"
    grep -Fq 'ksu_handle_vfs_read(&file, &buf, &count, &pos);' "$KERNEL_ROOT/fs/read_write.c" || die "manual vfs_read hook missing"
    grep -Fq 'ksu_handle_stat(&dfd, &filename, &flag);' "$KERNEL_ROOT/fs/stat.c" || die "manual stat hook missing"
    grep -Fq 'ksu_handle_sys_reboot(magic1, magic2, cmd, (void __user **)&arg);' "$KERNEL_ROOT/kernel/reboot.c" || die "manual reboot hook missing"
    grep -Fq 'ksu_handle_input_handle_event(&type, &code, &value);' "$KERNEL_ROOT/drivers/input/input.c" || die "manual input hook missing"
    grep -Fq 'ksu_handle_devpts(dentry->d_inode);' "$KERNEL_ROOT/fs/devpts/inode.c" || die "manual devpts hook missing"
    ok "SukiSU-Ultra non-GKI manual hook sites are present"
